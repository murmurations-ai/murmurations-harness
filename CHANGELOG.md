...
- **Claude adapter now captures tool calls** ([#430](https://github.com/murmurations-ai/murmurations-harness/issues/430)). The subscription-CLI claude adapter ran with `--output-format json`, which returns only the final result object and emits no `assistant`/`tool_use`/`user`/`tool_result` events — so `LLMResponse.toolCalls` was silently always empty in production (and per-turn `toolCallCount` always 0). It now runs `--output-format stream-json --verbose` (the parser already consumed NDJSON), captures each `tool_use`, and matches its `tool_result` outcome by `tool_use_id` to set a new `ToolCallResult.ok` (`is_error` → `false`). This restores tool visibility and is the foundation for `verifiedActions` (#364B). Behavior change to the core wake path for every subscription-CLI claude agent — validated by unit tests + a live `stream-json` format probe; the final `result`/`usage`/`session_id` parsing is unchanged.
- **A one-shot wake no longer orphans a co-rooted running daemon** ([#432](https://github.com/murmurations-ai/murmurations-harness/issues/432)). `murmuration start --now` (and `--once`) is designed to coexist with a live daemon at the same root, but its exit handler unconditionally `unlink`ed the shared `daemon.pid` and stopped the control socket (which `unlink`s `daemon.sock`) — even though, being one-shot, it never created either. The running daemon kept firing wakes but lost its socket + pidfile, becoming invisible to `list`/`attach`. One-shot runs now construct no control socket and clean up nothing (both gated on `!once`, matching the existing pidfile/socket-registration guards). Likely the root cause of recurring "daemon running but unreachable" states.
- **Coincident multi-cron sub-triggers no longer launch concurrent wakes for one agent** ([#435](https://github.com/murmurations-ai/murmurations-harness/issues/435)). With multiple `wake_schedule` triggers (#420), two `scheduled` sub-triggers (cron/interval) that elapse on the same tick fanned into independent timers that both reached `#runWake` for the same agent — racing the agent state store and the `runs/<agent>` writer (single owner per state file) and risking duplicate GitHub mutations. `Daemon.#handleWake` now keeps a per-agent in-flight set (added synchronously before the run's first `await`, cleared in its `finally`); a coincident `scheduled` wake for an agent already in flight is skipped (`daemon.wake.skipped`, `reason: "in-flight"`). Manual (`--now`) wakes are exempt — they always fire, matching the existing idle-skip exemption — so an operator override is never dropped. The missing guard predated #420 but was unreachable until per-agent multi-schedule made two coincident same-agent fires possible from config.

...
- [AgentSkills.io](https://agentskills.io) — portable skill format (SKILL.md) with Three-Tier Progressive Disclosure
...
The LLM layer (`@murmurations-ai/llm`) wraps Vercel AI SDK's `generateText()` behind our `LLMClient` interface, adding errors-as-values (`Result<T, LLMClientError>`), per-call cost tracking, and a 10-class error taxonomy. It also runs a parallel **subscription-CLI provider family** (ADR-0034) that subprocess-spawns the operator's locally-installed `claude`, `codex`, or `gemini` CLI — so operators with a Claude Pro/Max, ChatGPT, or Google subscription can run a murmuration at $0 marginal cost without an API key. Both routes implement the same `LLMClient` interface; the daemon doesn't branch on provider type. The MCP layer (`@murmurations-ai/mcp`) connects to MCP servers at wake time and converts their tools to Vercel-compatible `ToolDefinition[]` via `jsonSchema()`. The SkillScanner (`packages/core/src/skills/`) implements AgentSkills.io's Three-Tier Progressive Disclosure: scans `skills/` for `SKILL.md` frontmatter at wake time, injects `<available_skills>` XML into the system prompt, and agents load full skill content on demand via MCP `read` tool. Langfuse observability is opt-in: set two env vars and every LLM call reports spans automatically.

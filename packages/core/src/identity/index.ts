...
const buildBuiltinRoleDocument = (agentDir: string): string =>
  [
    ...
    "  governance items I've filed myself.",
    ...
    "",
    "- **Reports, digests, status updates, meeting minutes, chronicles** →",
    "  commit as files in the repository.",
    "- **Action items, Source-input requests, coordination, tensions** →",
    "  file as GitHub issues. **Close them when the action is done.**",
    "",
    "If the artifact's job is to be read, it's a file. If the artifact's",
    "job is to drive someone else to act, it's an issue. See",
    "`docs/CONVENTIONS-GITHUB-VS-FILES.md` in the harness repo.",
    "",
